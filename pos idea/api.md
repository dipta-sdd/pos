...
| SL No. | Method | Full Endpoint Path                                       | Purpose & Key Logic                                                                   | Request Payload (Body)                                                       |
|:------:|:------:|:---------------------------------------------------------|:--------------------------------------------------------------------------------------|:-----------------------------------------------------------------------------|
| 3.01   | `GET`  | `/settings/profile`                                      | Get the current vendor's profile (name, address, etc.).                               | *(None)*                                                                     |
| 3.02   | `PUT`  | `/settings/profile`                                      | Update the vendor's profile.                                                          | `{ "name", "description", "phone", "address", "currency", "timezone" }`        |
| 3.03   | `GET`  | `/settings/onboarding-status`                            | Get the vendor's onboarding progress checklist.                                       | *(None)*                                                                     |
| 3.04   | `POST` | `/settings/onboarding-status/complete-step`              | Mark a specific onboarding step as complete.                                          | `{ "step": "has_created_branch" }`                                           |
| 3.05   | `GET`  | `/settings/branches`                                     | List all branches for the vendor.                                                     | *(None)*                                                                     |
| 3.06   | `POST` | `/settings/branches`                                     | Create a new branch.                                                                  | `{ "name", "address", "phone", "description" }`                              |
| 3.07   | `GET`  | `/settings/branches/{branch}`                            | Get details of a single branch.                                                       | *(None)*                                                                     |
| 3.08   | `PUT`  | `/settings/branches/{branch}`                            | Update a specific branch.                                                             | `{ "name", "address", "phone", "description" }`                              |
| 3.09   | `DELETE`| `/settings/branches/{branch}`                            | Delete a branch.                                                                      | *(None)*                                                                     |
| 3.10   | `GET`  | `/settings/branches/{branch}/counters`                   | List billing counters for a specific branch.                                          | *(None)*                                                                     |
| 3.11   | `POST` | `/settings/branches/{branch}/counters`                   | Create a new billing counter for a branch.                                            | `{ "name" }`                                                                 |
| 3.12   | `GET`  | `/settings/branches/{branch}/counters/{counter}`         | Get a single billing counter's details.                                               | *(None)*                                                                     |
| 3.13   | `PUT`  | `/settings/branches/{branch}/counters/{counter}`         | Update a billing counter.                                                             | `{ "name" }`                                                                 |
| 3.14   | `DELETE`| `/settings/branches/{branch}/counters/{counter}`         | Delete a billing counter.                                                             | *(None)*                                                                     |
| 3.15   | `GET`  | `/settings/roles`                                        | List all roles for the vendor.                                                        | *(None)*                                                                     |
| 3.16   | `POST` | `/settings/roles`                                        | Create a new role with a payload of all boolean permissions.                          | `{ "name", "can_add_products": true, "can_view_reports": false, ... }`       |
| 3.17   | `GET`  | `/settings/roles/{role}`                                 | Get details of a single role, including all its permission values.                    | *(None)*                                                                     |
| 3.18   | `PUT`  | `/settings/roles/{role}`                                 | Update a role's name and all its permission values.                                   | `{ "name", "can_add_products": true, "can_view_reports": true, ... }`        |
| 3.19   | `DELETE`| `/settings/roles/{role}`                                 | Delete a role.                                                                        | *(None)*                                                                     |
| 3.20   | `GET`  | `/settings/permissions`                                  | Get the master list of all available permission names and descriptions.               | *(None)*                                                                     |
| 3.21   | `GET`  | `/settings/staff`                                        | List all staff (memberships) for the vendor.                                          | *(None)*                                                                     |
| 3.22   | `POST` | `/settings/staff/invite`                                 | Invite a new staff member via email.                                                  | `{ "email", "role_id", "branch_ids": [1, 2] }`                               |
| 3.23   | `GET`  | `/settings/staff/{membership}`                           | Get a single staff member's details, including assigned role and branches.            | *(None)*                                                                     |
| 3.24   | `PUT`  | `/settings/staff/{membership}`                           | Update a staff member's role and branch assignments.                                  | `{ "role_id", "branch_ids": [1, 3] }`                                        |
| 3.25   | `DELETE`| `/settings/staff/{membership}`                           | Deactivate or remove a staff member from the vendor.                                  | *(None)*                                                                     |
| 3.26   | `GET`  | `/settings/payment-methods`                              | List all payment methods.                                                             | *(None)*                                                                     |
| 3.27   | `POST` | `/settings/payment-methods`                              | Create a new payment method.                                                          | `{ "name", "is_active", "branch_id": null|1 }`                                 |
| 3.28   | `GET`  | `/settings/payment-methods/{method}`                     | Get a single payment method.                                                          | *(None)*                                                                     |
| 3.29   | `PUT`  | `/settings/payment-methods/{method}`                     | Update a payment method.                                                              | `{ "name", "is_active", "branch_id": null|1 }`                                 |
| 3.30   | `DELETE`| `/settings/payment-methods/{method}`                     | Delete a payment method.                                                              | *(None)*                                                                     |
| ...    | *and so on...*                                                   | ...                                                                                   | ...                                                                        |
| 3.45   | `GET`  | `/settings/receipts`                                     | Get the vendor's receipt customization settings.                                      | *(None)*                                                                     |
| 3.46   | `PUT`  | `/settings/receipts`                                     | Update the receipt settings.                                                          | `{ "header_text", "footer_text", "show_logo", "template_style" }`              |

*Note: Full CRUD for `taxes`, `promotions`, and `units-of-measure` follow the same explicit `GET (list)`, `POST`, `GET {id}`, `PUT {id}`, `DELETE {id}` pattern, totaling 15 more endpoints.*
