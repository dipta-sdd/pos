...
export default function DataExportsPage() {
  ...
  return (
    <PermissionGuard permission="can_export_data">
      <div className="p-6">
        <PageHeader title="Data Exports" description="Export your business data in various formats" />

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {exportTypes.map((type) => (
                <Card key={type.name} isPressable className="hover:border-primary transition-colors border-2 border-transparent">
                    <CardBody className="p-6 flex items-center justify-between">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-gray-500">Format: {type.format}</p>
                            </div>
                        </div>
                        <Button color="primary" variant="flat" startContent={<FileDown className="w-4 h-4" />}>
                            Download
                        </Button>
                    </CardBody>
                </Card>
