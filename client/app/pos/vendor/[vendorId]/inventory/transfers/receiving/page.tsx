...
const columns: Column[] = [
  ...
  { name: "SOURCE", uid: "from_branch", sortable: false },
...
export default function IncomingTransfersPage() {
  ...
  const getBulkActions = (): BulkAction[] => {
    ...
    const allInTransit = selectedItems.every((i) => i.status === "in_transit");
    const allShipped = selectedItems.every((i) => i.status === "shipped");
  ...
  const fetchItems = async (page: number) => {
    ...
    try {
      ...
      const response: any = await api.get(`/stock-transfers/receiving`, {
        params,
      });
  ...
  const renderCell = useCallback(
    (item: StockTransfer, columnKey: React.Key) => {
      switch (columnKey) {
        ...
        case "actions":
          const canEdit = item.status === "requested";
          ...
          return (
            <div className="flex items-center justify-end gap-2">
              <Button
                ...
                onPress={() =>
                  router.push(
                    `/pos/vendor/${vendor?.id}/inventory/transfers/receiving/${item.id}`,
                  )
                }
              >
                {canEdit ? (
  ...
  return (
    <PermissionGuard permission="can_view_stock_and_inventory">
      <div className="p-6">
        <PageHeader
          description="Items coming to your branches"
        ...
        >
          <Button
            ...
            onPress={() =>
              router.push(
                `/pos/vendor/${vendor?.id}/inventory/transfers/receiving/new`,
              )
            }
          >
            Request Stock
