...
export default function CreateSendingTransferForm() {
  ...
  const onSubmit = async (data: any) => {
    try {
      ...
      router.push(`/pos/vendor/${vendor?.id}/inventory/transfers/sending`);
    } catch (error) {
      ...
      <form className="space-y-8" onSubmit={handleSubmit(onSubmit)}>
              ...
              <div className="flex items-center justify-between">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full">
                  <Select
                    ...
                    errorMessage={
                      !watchFromBranch && "Please select a source branch"
                    }
                  >
                    {allBranches
                  ...
                  >
                    {allBranches
                      .filter((b) => b.id !== Number(watchFromBranch))
                      .map((branch) => (
                        <SelectItem key={branch.id.toString()}>
                          {branch.name}
                        </SelectItem>
        ...
        <div className="flex justify-end gap-3 pb-8">
          <Button
            ...
            onPress={() =>
              router.push(
                `/pos/vendor/${vendor?.id}/inventory/transfers/sending`,
              )
            }
          >
            Cancel
