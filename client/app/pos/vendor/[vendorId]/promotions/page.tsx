...
const columns: Column[] = [
  ...
  { name: "DISCOUNT TYPE", uid: "discount_type", sortable: true },
  { name: "VALUE", uid: "discount_value", sortable: true },
...
const INITIAL_VISIBLE_COLUMNS = ["name", "discount_type", "discount_value", "is_active", "start_date", "end_date"];
...
export default function PromotionsPage() {
  ...
  const fetchItems = async (page: number) => {
    ...
    setLoading(true);
    try {
      const response: any = await api.get(`/promotions`, {
        params: {
          page,
          per_page: perPage,
          vendor_id: vendor.id,
  ...
  const renderCell = useCallback((item: Promotion, columnKey: React.Key) => {
    switch (columnKey) {
      ...
      case "start_date":
        return item.start_date ? formatDateTime(item.start_date) : "N/A";
      case "end_date":
        return item.end_date ? formatDateTime(item.end_date) : "N/A";
      case "discount_value":
        return typeof item.discount_value === "number"
          ? item.discount_value.toFixed(2)
          : item.discount_value;
      default:
        return (item as any)[columnKey as keyof Promotion];
    }
  }, []);
