...
import ExpenseCategoryForm from "./_components/ExpenseCategoryForm";
...
const INITIAL_VISIBLE_COLUMNS = ["name", "created_at", "actions"];
...
export default function ExpenseCategoriesPage() {
  ...
  const fetchItems = async (page: number) => {
    ...
    setLoading(true);
    try {
      const response: any = await api.get(`/expense-categories`, {
        params: {
          page,
          per_page: perPage,
          vendor_id: vendor.id,
  ...
  const renderCell = useCallback((item: ExpenseCategory, columnKey: React.Key) => {
    switch (columnKey) {
      case "created_at":
      ...
      default:
        return (item as any)[columnKey as keyof ExpenseCategory];
    }
  }, []);
  ...
  return (
    <PermissionGuard permission="can_manage_expenses">
      <div className="p-6">
        <PageHeader description="Manage expense categories" title="Expense Categories">
